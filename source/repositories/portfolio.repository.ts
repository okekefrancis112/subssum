...
class PlanRepository {
    ...
    public async getOne(
        query: FilterQuery<IPlanDocument>
    ...
    public async getAll(
        ...
        return Plan.find(query).exec() as Promise<IPlanDocument[]>;
    }
        ...
        const plan_pipeline = [
...

            {
                $project: {
                    ...
                    investment_category: {
                        $ifNull: ["$investment.investment_category", "fixed"],
                    },
                    payment_style: "$investment.investment_occurrence",
                    cash_dividend: { $sum: "$investment.cash_dividend" },
                                ...
                                $multiply: [
                                    {
                                        $divide: [
                                            // Current Date
                                            {
                                                $subtract: [
                                                    "$$NOW",
                                                    "$investment.last_dividends_date",
                                                ],
                                            },
...

            {
                $group: {
                    ...
                    payment_style: { $first: "$payment_style" },
                    cash_dividend: { $first: "$cash_dividend" },
                    plan_occurrence: { $first: "$plan_occurrence" },
                    no_tokens: { $first: "$no_tokens" },
                    total_amount: { $sum: "$investment.amount" },
                    holding_period: { $first: "$listing.holding_period" },
                    createdAt: { $first: "$createdAt" },
                    asset_name: { $first: "$listing.project_name" },
                    amount_invested: { $sum: "$amount_invested" },
                    expected_earnings: { $sum: "$expected_earnings" },
                    investment_type: { $first: "$investment_type" },
...

            {
                $project: {
                    ...
                    expected_earnings: format_query_decimal(
                        "$expected_earnings",
                        100
                    ),
                    investment_category: {
        ...
        const plan_pipeline = [
...

            {
                $group: {
                    ...
                    asset_name: { $first: "$listing.project_name" },
                    amount_invested: { $sum: "$amount_invested" },
                    expected_earnings: { $first: "$expected_earnings" },
                    monthly_returns: { $first: "$monthly_returns" },
                    status: { $first: "$status" },
                    plan_status: { $first: "$plan_status" },
                },
            },
...

            {
                $project: {
                    ...
                    monthly_returns: format_query_decimal(
                        "$monthly_returns",
                        100
                    ),
                    plan_occurrence: 1,
    ...
    public async getUserPlans(
        ...
        const myDateFrom = convertDate(dateFrom);
        const myDateTo = convertDate(dateTo);
        let period = String(query.period) || "today";
        let timeFilter = {};
        let filter = {};
        let days;
    ...
    public async findV4(
        ...
        let plan_type = String(query.plan_type) || "all";
    ...
    public async findPaymentStyleChart({
        ...
        const payment_style_chart = await this.findAggregate(
            payment_style_pipeline
        );
