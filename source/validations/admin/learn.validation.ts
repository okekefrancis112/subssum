...
export async function validateCreateLearn(
  req: ExpressRequest,
  res: Response,
  next: NextFunction
  ...
  const schema = Joi.object().keys({
    ...
    video_name: Joi.string().required(),
...
export async function validateLearnId(req: ExpressRequest, res: Response, next: NextFunction) {
  if (!mongoose.Types.ObjectId.isValid(req.params.learn_id))
    return ResponseHandler.sendErrorResponse({
      res,
      code: HTTP_CODES.NOT_FOUND,
      error: 'Invalid Learn ID',
    });
