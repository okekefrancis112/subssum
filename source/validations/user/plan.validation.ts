...
export async function validateCreateInvestPlanWallet(
  req: ExpressRequest,
  res: Response,
  next: NextFunction
  ...
  const schema = Joi.object()
    .keys({
      ...
      intervals: Joi.string()
        ...
        .required(),
      plan_occurrence: Joi.string().valid(IPortfolioOccurrence.ONE_TIME_PAYMENT).required(),
...
export async function validateCreateSavingPlanWallet(
  req: ExpressRequest,
  res: Response,
  next: NextFunction
): Promise<Response | void> {
...
export async function validateTopUpInvestPlanWallet(
  req: ExpressRequest,
  res: Response,
  next: NextFunction
): Promise<Response | void> {
  ...
  const schema = Joi.object()
    .keys({
      ...
      plan_occurrence: Joi.string()
        .valid(IPortfolioOccurrence.ONE_TIME_PAYMENT, IPortfolioOccurrence.RECURRING)
        .required(),
      duration: Joi.number().required(),
      amount: Joi.number().required(),
...
export async function validateTopUpInvestPlan(
  req: ExpressRequest,
  res: Response,
  next: NextFunction
): Promise<Response | void> {
  ...
  const schema = Joi.object()
    .keys({
      amount: Joi.number().required(),
      payment_gateway: Joi.string()
        .valid(IPaymentGateway.FLUTTERWAVE, IPaymentGateway.MONO, IPaymentGateway.PAYSTACK)
        .required(),
      channel: Joi.string()
        .valid(ITransactionMedium.BANK, ITransactionMedium.CARD, ITransactionMedium.DIRECT_DEBIT)
        .required(),
...
export async function validateCustomSavings(
  ...
  const schema = Joi.object()
    .keys({
      ...
      goal_target: Joi.number().required(),
      interval: Joi.string()
        .valid(IPortfolioIntervals.DAILY, IPortfolioIntervals.WEEKLY, IPortfolioIntervals.MONTHLY)
        .required(),
    })
    .unknown();
...
export const planIsExist = async (
  req: ExpressRequest,
  plan_id: Types.ObjectId,
  auth: IUserDocument
) => {
  const plan = await planRepository.getOne({ _id: plan_id });
